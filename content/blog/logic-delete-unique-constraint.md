---
title: 逻辑删除与唯一约束冲突的工程化解决方案
date: 2026-01-10
category: 数据库设计
excerpt: 在生化设计变更管理系统开发中，解决逻辑删除与唯一约束冲突的技术难题。通过「业务字段+deleted_at」唯一键设计实现历史版本审计和"删除-重建"循环，并封装LogicDeleteHelper规避MyBatis-Plus陷阱，保证系统稳定性。
tags:
  - MySQL
  - 逻辑删除
  - 唯一约束
...
> **项目背景**: 新产业生化设计变更管理系统（24张表，14状态×5流程通道）  
> **核心问题**: 审计要求保留历史 + 业务要求唯一性 + 用户可能反复操作  
> **技术方案**: 「业务字段 + deleted_at」唯一键 + LogicDeleteHelper 统一封装
...
### 为什么不用 del_flag 参与唯一键？

`del_flag` 是布尔值，同一业务键多次删除后，所有历史记录的 `del_flag` 都是 1，导致唯一键冲突。而 `deleted_at` 时间戳天然唯一，能够区分不同版本。

### 为什么不分表（在线表 + 历史表）？

分表方案确实能提供更好的查询性能，但需要考虑：
- **数据同步复杂度**：每次删除需要将数据从在线表迁移到历史表
- **事务一致性**：跨表操作增加事务复杂度
- **维护成本**：需要维护两套表结构

在当前数据规模下（单表百万级以内），「业务字段 + deleted_at」唯一键方案已经能够满足性能要求，改动成本更低。

### 未来优化方向

如果数据规模继续增长，可以考虑：
1. **部分索引**：利用支持过滤索引的数据库（如 PostgreSQL），为 `WHERE deleted_at IS NULL` 创建唯一索引
2. **冷热分离**：历史数据归档到独立表或存储系统
3. **CDC 同步**：通过 Change Data Capture 异步同步历史表

---

## 📚 总结

本文分享了在企业级系统开发中，如何系统性地解决"逻辑删除 + 唯一约束"冲突问题：

### 核心方案

**数据库层**：「业务字段 + deleted_at」唯一键设计
- 利用 MySQL NULL 特性，在线记录保证唯一性
- 历史记录通过时间戳区分版本

**应用层**：LogicDeleteHelper 统一封装
- 规避 MyBatis-Plus Lambda 父类字段解析问题
- 脱离自动填充的不确定性
- 保证逻辑删除行为的一致性

### 技术价值

1. **满足合规要求**：完整保留历史数据用于审计
2. **保证数据一致性**：数据库层唯一约束强制业务规则
3. **支持复杂业务流程**：允许「删除-重建」循环操作
4. **工程化封装**：通过 Helper 降低维护成本和出错风险

### 技术栈

Spring Boot + MyBatis-Plus + MySQL（主从复制）

### 涉及技术点

- 逻辑删除与唯一约束设计
- MySQL NULL 在唯一键中的行为
- MyBatis-Plus update 机制
- Lambda 表达式陷阱与最佳实践
- 工程化封装与代码规范
